...
where a genes may have zero counts under many samples if the gene is not 
expressed. 
...
both conditions, permuting sample labels may group such samples under one 
condition by chance and hence produce a standard deviation smaller than the 
...
However, due to some limitations, two points must be declared:
\begin{itemize}
...
LU TUMOR VASCULATURE UP used in \cite{Rahmatallah2014} to demonstrate the 
GSNCA. 
