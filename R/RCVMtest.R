    ...
    function(object, group, mst.order=1, nperm=1000, pvalue.only=TRUE)
{
    ...
    { 
        ...
        term1 <- sum(((which(randperm[radial_ranking] == "green")/(nv-nv1)) - 
           (c(1:nv1)*((1/nv1)+(1/(nv-nv1)))))^2)
        term2 <- sum(((which(randperm[radial_ranking] == "red")/nv1) - 
           (c(1:(nv-nv1))*((1/nv1)+(1/(nv-nv1)))))^2)
        T_perm[itr] <- ((nv1*(nv-nv1)) / (nv1+(nv-nv1))^2) * (term1 + term2)
    }

    term1 <- sum(((which(domain[radial_ranking] == "green")/(nv-nv1)) - 
       (c(1:nv1)*((1/nv1)+(1/(nv-nv1)))))^2)
    term2 <- sum(((which(domain[radial_ranking] == "red")/nv1) - 
       (c(1:(nv-nv1))*((1/nv1)+(1/(nv-nv1)))))^2)
