    ...
    function(object, group, mst.order=1, nperm=1000, pvalue.only=TRUE)
{
    ...
    { 
        ...
        for (i in 1:nv)
        {
        ri <- sum((radial_ranking[1:i]*(randperm[radial_ranking[1:i]] == "green"))^0.25) / sum((radial_ranking*(randperm[radial_ranking] == "green"))^0.25)
        si <- sum(randperm[radial_ranking[1:i]] == "red") / (nv-nv1)
        di[i] <- ri-si
        }
    D_perm[itr] <- sum(di)
    }
    ...
    for (i in 1:nv)
    {
        ri <- sum((radial_ranking[1:i]*(domain[radial_ranking[1:i]] == "green"))^0.25) / sum((radial_ranking*(domain[radial_ranking] == "green"))^0.25)
        si <- sum(domain[radial_ranking[1:i]] == "red") / (nv-nv1)
        di[i] <- ri-si
    }
    D_obs <- sum(di)
