    ...
    function(object, group, mst.order=1, nperm=1000, pvalue.only=TRUE)
{
    ...
    RAD_perm[itr] <- sum(di) / (nv1 * (nv-nv1))
    }
    ...
    RAD_obs <- sum(di) / (nv1 * (nv-nv1))
    pvalue <- (sum(RAD_perm >= RAD_obs) + 1) / (length(RAD_perm) + 1)    
    if(pvalue.only) return(pvalue)
    if(!pvalue.only) return(list("statistic"=RAD_obs,"perm.stat"=RAD_perm,"p.value"=pvalue))
}
